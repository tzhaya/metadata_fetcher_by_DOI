...
DOIからUnpaywallのREST APIを検索してISSNとOAの場合のライセンスを取得し、このISSNからOpen policy finderを検索して論文掲載誌のOAポリシーが記載されたURLを取得します。
...
最新リリース (ver.2.0) は [GitHub Releases](https://github.com/tzhaya/metadata_fetcher_by_DOI/releases/tag/v2.0) からダウンロードできます。

## ファイルの説明
- **metadata_fetcher_by_DOI.xlsx**
  - 通常版です。
  - Unpaywall、Open policy finder (Formerly Sherpa services)、「日本の学協会の著作権ポリシー確認ツール」経由でSCPJの情報を取得します。
  - Open Policy FinderのAPIキーの入手が必要です。2026年4月末以降に入手できるようです。すでにSherpa ServicesのAPIキーをお持ちの方は引き続き利用できます。
...
- Open policy finderのAPIキー
  - Sherpa Serices APIをお持ちの場合は、そのまま利用できます。
  - Sherpa SericesからOpen policy finderへの移行に伴い、仕様が変更となったようです。
  - APIキーは、現在の legacy Sherpa Services API からの以降が終わった2026年4月以降に新規の取得ができるようです。詳しくは https://openpolicyfinder.jisc.ac.uk/help/developers/how-to-request-a-new-api-key をご覧ください。
  - お持ちでない場合は、Sherpa Servicesを使用しない metadata_fetcher_by_DOI_ss_html.xlsx をご利用ください。

## 初期設定
- Open Policy Finder の API キーを Power Query のパラメーターに設定します。
  1. Power Query エディタを開きます（「データ」タブ → 「クエリと接続」→ 対象クエリをダブルクリック）。
  2. 左のクエリ一覧から `OpfApiKey` をクリックします。
  3. 「現在の値」フィールドに取得した API キーを入力します。
  4. 「閉じて読み込む」をクリックします。

![OpfApiKey パラメーターの設定](docs/parameter_opf.png)

- UnpaywallのAPIキーの設定が必要です。メールアドレスを設定します。
- 初回の検索の際に資格情報の入力を求められます。「Web API」を選択して設定します。
...
## 使い方
1. シート「DOI」の左端のテーブルに、検索したい文献のDOIを入力します。形式は 10.1038/s41587-024-02248-6 のようにしてください。`https://doi.org/...` や `DOI:...` の形式も自動で正規化します。
2. Excelメニューの「クエリ」を開き「クエリと接続」にある「すべて更新」を押します。
3. 入力したDOIを元に、Unpaywallを検索してISSNを、このISSNからOpen Policy Finderと[日本の学協会の著作権ポリシー確認ツール](https://app.lib.shimane-u.ac.jp/policy_checker/scpj.php)（以下、「Policy Checker (SCPJ) 」といいます。）を検索して以下の表にある情報を取得します。項目名に UnPayWall とあるものがUnpaywallから取得したデータです。件数が多いと、取得には数秒以上かかります。
4. Unpaywallに登録されていないDOI、またはISSNが Open Policy Finder やPolicy Checker (SCPJ)でヒットしない場合は、セルに入力がされません。
...
3. 結果のテーブルを作成します。
4. カスタム関数 getOpenpolicyfiner に、結果のテーブルに含まれるISSN-LとAPIキーを渡します。この関数は、ISSNをOpen Policy Finderに送信し、取得したJSONから詳細情報が記載されたURL（のみ）を返します。
5. カスタム関数 getOpenpolicyfiner から返ってきたURLを、結果のテーブルに追加します。
...
## その他
- 主に使用するクエリ DOI と、カスタム関数 getUnpaywall、getOpenpolicyfiner、getSCPJdata、getSCPJhtml をエクスポートして source フォルダに置きました。
- その他の項目の取得が必要な場合は、クエリ DOI とカスタム関数を適宜書き換えてご利用ください。
- 項目の追加・変更は、取得用のカスタム関数関数と、項目を展開するクエリ DOI 両方の書き換えが必要です。
- 存在しないDOI、Unpaywall未登録のDOI、ISSNが Open Policy Finder やPolicy Checker (SCPJ)でヒットしない場合は入力がない（null値または"N/A"など）と表示されます。
...
- Open policy finder (Formerly Sherpa services)
  ...
  -  The API https://openpolicyfinder.jisc.ac.uk/help/developers
...
## 更新
 - 2026/03/01
   - Open policy finderの新APIに対応しました。APIキーの送信方法が従来のクエリパラメータから、HTTPのリクエストヘッダ `x-api-key` での送信に変わりました。このため、APIキーの設定方法を変更しました。
   - DOIの入力を、以下のいずれの形式でも `10.1038/xxx` に自動で正規化するように変更しました。 先頭・末尾の空白も除去します。
     - https://doi.org/10.1038/xxx	
     - http://doi.org/10.1038/xxx	
     - DOI:10.1038/xxx	（大文字小文字不問）
...
## To Do
- ~~Open policy finder APIへの対応~~ [issue #5](https://github.com/tzhaya/metadata_fetcher_by_DOI/issues/5) で対応。
...
- 2026/03/01 にリリースした ver.2.0以降の各ファイルは [CC0 1.0 Universal (CC0 1.0) Public Domain Dedication](https://creativecommons.org/publicdomain/zero/1.0/) の下で公開されています。詳細は [LICENSE](LICENSE) ファイルを参照してください。
